...
cs['bmymager2'] = cs.bmymager.apply(lambda x: str(pd.to_datetime(x)))
...
# 	new_dict_ = new_dict.copy()
#
# 	for m in match_list:
# 		for p in phrases:
# 			if any([(w in p) for w in m[1]]):
# 				new_dict_[m[0]].append(p)
...
df_list['url'] = 'https://zh.wikipedia.org/wiki/' + df_list.name
df_list['baidu'] = 'https://baike.baidu.com/item/' + df_list.name
df_list.to_excel('Data/市领导数据/二次爬虫-edu_etc/爬到75人信息整理.xlsx', index=False)
